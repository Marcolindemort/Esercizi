...
let me = {name: "Marco", surname: "Mandolini", age: "27"}
...
me.skills = ["Javascript anche se non è vero"]
...
let splitMe = function (string) {
  let paroleSplittate = string.split(" ");
  parole.push(paroleSplittate)
}

splitMe("Io non amo Javascript")
...
let deleteOne = function (string, booleano) {
  ...
  } else {console.log ("Esercizio 4:", string.slice(0, -1))}
}
...
let onlyLetters = function (string) {
  let numberLess = string.replace(/\d+/g, '');
  console.log ("Esercizio 5:", numberLess)
}
...
let whatDayIsIt = function () {
  ...
  const giornoEsatto = new Date();
  const giornoPreciso = giornoEsatto.getDay();
  return giorni [giornoPreciso -1]
}
...
/*let isTodayMyBirthday = function (compleanno) {
  let oggi = new Date();
  let meseCompleanno = oggi.getMonth();
  let giornoCompleanno = oggi.getDay();
  compleanno = giornoCompleanno + meseCompleanno
}
*/
...
console.log ("Esercizio 17:", searchByTitle (movies,"Lord"))
...
const searchAndDivide = (movies, string) => {
  const match = movies.filter(movie => movie.Title.includes (string));
  const unmatch = movies.filter(movie => !movie.Title.includes(string));
  return {match, unmatch};
}
console.log ("Esercizio 18:", searchAndDivide (movies,"Lord"))
...
let svuota = function () {
  let lista = document.getElementsByTagName("myList");
  lista.innerHTML = ""
} 
